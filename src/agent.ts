...
export const declarations: FunctionDeclaration[] = [
  {
    name: "get_scene",
    description:
      "Read the current board, shape IDs, sizes and centre coordinates.",
    parameters: { type: Type.OBJECT, properties: {} },
  },
  {
    name: "move_shape",
    description:
      "Move a movable shape to absolute centre coordinates. Returns actual resulting state or an error.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        shape_id: string("Shape ID from get_scene"),
        x: { type: Type.NUMBER },
        y: { type: Type.NUMBER },
      },
  ...
  },
  {
    name: "check_relation",
    description:
      "Verify a relationship using actual geometry. Entire shapes must satisfy it; touching is allowed.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        subject_id: string("Subject shape ID"),
        relation: {
          type: Type.STRING,
          enum: ["inside", "left_of", "right_of", "above", "below"],
        },
        target_id: string("Target shape ID"),
      },
      required: ["subject_id", "relation", "target_id"],
    },
  },
];
export type Event = {
  kind: "request" | "model" | "tool" | "observation" | "answer" | "error";
  title: string;
  data?: unknown;
};
...
export function friendlyError(error: unknown): string {
  ...
  if (
    ...
    return "The provider rejected the key. Check your key and its API permissions in Settings.";
  if (code === 429)
    return "Provider quota or rate limit reached. Check your account quota, then try again later.";
  if (code === 402)
    return "The provider requires credits. Check your account balance.";
  if (code === 404)
    return "This model is unavailable. Choose an accessible model ID in Settings.";
  if (code === 400 || code === 422)
    return "The provider rejected the request. Check that this model supports tools and the selected API.";
  return "The request failed or timed out. Check your connection, browser access, and model availability, then try again.";
}
export class Agent {
  history: Content[] = [];
  running = false;
  ...
  constructor(
    ...
    private status: (label: string, waiting: boolean) => void,
  ...
  async run(prompt: string, send: Transport, model: string) {
    ...
    try {
      for (let call = 1; call <= 10; call++) {
        ...
        this.emit({
          kind: "request",
          title: `Request ${call} → ${model}`,
          data: send.describe?.(this.history) ?? {
            model,
            systemInstruction: instructions,
        ...
        this.status("Waiting for the model…", false);
        ...
        const calls = content.parts.flatMap((p) =>
        ...
        for (const tool of calls) {
          ...
          this.changed();
          if (tool.name === "move_shape" && result.ok)
      ...
      this.emit({
        kind: "error",
        title:
          "Stopped at the 10-request limit. Completed movements remain on the board.",
      });
      ...
      const message = signal.aborted
        ? "Stopped. Completed movements remain; this unfinished turn was removed from model history."
        : friendlyError(error);
